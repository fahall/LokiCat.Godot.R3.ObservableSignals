...
""");
            ...
            foreach (var field in group)
            {
                ...
                var customSignalName = rxSignalAttr.ConstructorArguments.FirstOrDefault().Value as string;
                var fieldName = fieldSymbol.Name;
                var propertyName = ToPropertyName(fieldName);
                var signalName = string.IsNullOrEmpty(customSignalName) ? propertyName : customSignalName;
                signalName = SanitizeSignalName(signalName);
    ...
    private bool _{{propertyName}}Connected = false;

    private void Ensure{{propertyName}}Connected()
    {
        if (_{{propertyName}}Connected) return;
        _{{propertyName}}Connected = true;
